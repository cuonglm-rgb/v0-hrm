...
function isWeekend(
  ...
  if (day === 6) return isSaturdayOff(date, saturdaySchedules, saturdayConfig)
  return false
}
...
// Phiếu đi muộn/về sớm làm bù đã duyệt cho ngày
function getLateEarlyMakeupForDate(
  date: string,
  ...
  const req = leaveRequests.find(
    (r) =>
      ...
      r.status === "approved" &&
      (r.request_type as { code?: string })?.code === "late_early_makeup" &&
      r.request_date === date &&
      r.to_time
  )
  return req || null
...
function getSpecialDayForDate(
  ...
  // Nếu có chọn nhân viên cụ thể, kiểm tra xem nhân viên có trong danh sách không
  if (day.assigned_employees && day.assigned_employees.length > 0) {
...
export function AttendancePanel({ attendanceLogs, shift, leaveRequests = [], officialDate = null, holidays = [], specialDays = [], saturdaySchedules = [], employeeId, editable = false }: AttendancePanelProps) {
  ...
  }, [attendanceLogs, filterMonth, filterYear, filterStatus, shift, specialDays, employeeId, leaveRequests])
  ...
  }, [filteredLogs, filterMonth, filterYear, leaveRequests, holidays, specialDays, saturdaySchedules, filterStatus, shift, employeeId, saturdayConfig])
                ...
                {paginatedAttendance.length === 0 ? (
                  ...
                  paginatedAttendance.map(({ date, log, leaveRequest, holiday, specialDay }) => {
                    ...
                    // Nếu không có phiếu nghỉ nửa ngày, kiểm tra dựa vào giờ check in/out
                      ...
                      // Xác định giờ tan ca hiệu lực (có thể bị ghi đè bởi ngày đặc biệt)
